...
void f2() {
	...
	int totalsize = sizeof(b); // 24 바이트
	printf("%d\n", totalsize); // 배열 b 전체 사이즈 ( 24바이트 )
	int bsize = sizeof(b[0]); // 0번째 행의 사이즈
	printf("%d\n", bsize); // 12바이트

	printf("%d\n", sizeof(b[0][0])); // 0행 0번째 원소의 사이즈 ( 4바이트 )
	printf("%d\n", sizeof(b) / sizeof(b[0])); // 전체 크기 / 한 행 크기 ( 24바이트 / 12 바이트 = 2 )
	printf("%d\n", sizeof(b[0]) / sizeof(b[0][0])); // 한행 크기 / 한 원소의 크기 ( 12바이트 / 4바이트 = 3 )
	printf("==================\n");
}



int main() { // main 함수 1개, 처음 시작할때 main부터 시작
	//f1(); 함수 호출에는 소괄호가 필요
	//f2();
	f3(); // 함수 호출
